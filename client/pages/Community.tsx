...
function Comments({ postId }: { postId: number }) {
  const userId = getUserId();
  const qc = useQueryClient();
  const { data } = useQuery<{ comments: Cmt[] }>({
    queryKey: ["comments", postId],
    queryFn: () => fetchJSON(`/api/community/posts/${postId}/comments`),
  });
  ...
  return (
    <div className="mt-3 space-y-2">
      <div className="flex gap-2">
        ...
        />
        <Button onClick={() => m.mutate()} disabled={!text.trim()}>
          Reply
        </Button>
      </div>
...
function Feed() {
  const { data, isLoading } = useQuery<{ posts: Post[] }>({
    queryKey: ["feed"],
    queryFn: () => fetchJSON("/api/community/feed"),
  });
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Share progress, ask questions, and celebrate wins
        </CardDescription>
      </CardHeader>
...
function HelpRequests() {
  const qc = useQueryClient();
  const { data } = useQuery<{ requests: HelpRequest[] }>({
    queryKey: ["help"],
    queryFn: () => fetchJSON("/api/community/help-requests"),
  });
...
function ChatRooms() {
  const userId = getUserId();
  const { data: rooms } = useQuery<{ rooms: Room[] }>({
    queryKey: ["rooms"],
    queryFn: () => fetchJSON("/api/community/rooms"),
  });
    ...
    <Card>
      ...
      <CardContent>
        ...
        {roomId ? (
          ...
          <div className="text-sm text-muted-foreground">Select a room</div>
        )}
      </CardContent>
    </Card>
  );
...
function ChatRoom({ roomId, userId }: { roomId: number; userId: string }) {
  ...
  const { data } = useQuery<{ messages: Message[] }>({
    ...
    queryFn: () =>
      fetchJSON(
        `/api/community/rooms/${roomId}/messages${since ? `?since=${encodeURIComponent(since)}` : ""}`,
      ),
...
function Leaderboard() {
  const { data } = useQuery<{ leaderboard: LeaderboardEntry[] }>({
    ...
    queryFn: () => fetchJSON("/api/community/leaderboard"),
  });
  return (
    <Card>
      <CardHeader>
...
export default function Community() {
  const [tab, setTab] = useState<"feed" | "help" | "chat" | "leaderboard">(
    "feed",
  );
