...
async function extractText(buffer: Buffer, mimetype: string, originalname: string): Promise<string> {
  if (mimetype === "application/pdf" || originalname.endsWith(".pdf")) {
    const data = await unpdfExtract(new Uint8Array(buffer));
    return typeof data.text === "string" ? data.text : (data.text as any[]).join(" ");
  } else if (
...
export const handleResumeAnalysis: RequestHandler = async (req, res) => {
  try {
    ...
    console.log("[Resume] ENV GROQ_API_KEY present:", !!process.env.GROQ_API_KEY);
    console.log("[Resume] Endpoint:", "https://api.groq.com/openai/v1/chat/completions");
    console.log("[Resume] Model:", "llama-3.3-70b-versatile");
    ...
    console.log("[Resume] Raw Groq response:", responseText);
